...
public class Controleur extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    request.setAttribute("modelPets", pets);
      ...
      List < Prix > prix = metierPrix.getPrix();
    request.setAttribute("modelPrix", prix);
    ...
    if (path.contains("/details.do")) {
    	 i=1;
       request.setAttribute("hh", i);
      int pet_id = Integer.parseInt(request.getParameter("id"));
      int prix_id = Integer.parseInt(request.getParameter("id_prix"));
      for (Pet p: pets) {
      ...
      for (Prix pr: prix) {
        if (pr.getId_prix()== prix_id) {
          request.setAttribute("prix_selected", pr);
        }
      }
